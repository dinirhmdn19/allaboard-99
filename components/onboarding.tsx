...
useEffect(() => {
  ...
  if (step.kind === 'linkedin') return <><h1 className="text-4xl font-bold tracking-tight">Update your LinkedIn profile.</h1><p className="mt-4 max-w-2xl text-lg leading-relaxed text-ink/70">Share your 99er journey with a fresh profile banner or a SuperTwibbon. Your People Team badge can live here when it’s ready.</p><div className="mt-7 grid gap-4 sm:grid-cols-2"><ExternalLink href="https://sites.google.com/99.co/99ers-home/brand-assets-templates">Open banner resources</ExternalLink><ExternalLink href="https://www.supertwibbon.com/99ersLinkedInProfile">Open SuperTwibbon</ExternalLink></div><div className="mt-7 grid aspect-[3/1] place-items-center rounded-3xl border-2 border-dashed border-ink/20 bg-ink/[.02] text-sm text-ink/55">People Team badge preview</div>{step.externalUrl && <ExternalLink href={step.externalUrl} className="mt-6">Edit your LinkedIn profile</ExternalLink>}</>
  const details: Record<number, { title: string; copy: string; action: string }> = { 1: { title: 'Welcome aboard.', copy: 'One person. One journey. One step at a time.', action: '' }, 7: { title: 'Update your Slack profile.', copy: 'To help everyone recognise and connect with you easily, please update your profile photo, add your manager, and include your role and department.', action: t.openSlack }, 10: { title: 'Set up your email signature.', copy: 'Use the shared template to make your email recognisably 99.', action: 'Open email signature template' }, 11: { title: 'Add your onboarding meetings to Google Calendar.', copy: 'Keep the important moments in your first few weeks close at hand.', action: 'Open Google Calendar' }, 12: { title: 'You’re almost all set.', copy: 'From here, 99ers Home has everything else — benefits, forms, and the processes you may use along the way.', action: 'Explore 99ers Home' } }
